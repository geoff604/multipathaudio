...
class net.onepixelout.audio.Player
{
	...
	// For crossfades
	private var _crossFadeInSound:Sound; // the sound to fade in
	private var _crossFadeClearID:Number;
	private var _beginCrossFadeClearID:Number;
	static private var CROSS_FADE_TIME:Number = 250; // time for the crossfade in ms.
	static private var CROSS_FADE_INTERVAL_TIME:Number = 50;
	private var _crossFadeTimeLeft:Number;
	private var _crossFadeVolumeStep:Number;
	...
	private var _isLoadingXML:Boolean = false;
	private var _preloadOnFinishXML:Boolean = false;
	private var _inMultiPathMode = false;
	...
	* Preloads all tracks
	*/
	public function preloadTracks(callback:Function):Void
	{
		_preloadedCallback = callback;
		
		if (_isLoadingXML)
		{
			_preloadOnFinishXML = true;
			return;
		}	
		_doPreload();
	}
	
	private function _doPreload()
	{
		var trackCount:Number;
		trackCount = _playlist.getTrackCount();
		
		for(var i:Number = 0;i<trackCount;i++)
		{
			_playlist.getTrackAt(i).preload(_options.checkPolicy, 
											Delegate.create(this,trackLoaded));
		}
	}
	
	private function trackLoaded(success:Boolean)
	{
		var allLoaded:Boolean = true;
		for(var i:Number = 0;i<_playlist.getTrackCount();i++)
		{
			if (!_playlist.getTrackAt(i).isFullyLoaded())
			{
				allLoaded = false;
			}
		}
		if (allLoaded && _preloadedCallback != undefined)
		{
			_preloadedCallback();
			_preloadedCallback = undefined;
		}
	}
	...
	* Starts the player
	*/
	public function play(trackIndex:Number, broadcast:Boolean):Void
	{
		// If already playing, do nothing
		if(_state == PLAYING && (trackIndex == undefined || _playlist.getCurrentIndex() == trackIndex))
		{
			return;
		}	
		...
		_playhead = currentTrack.getSound();
		...
		// Tell anyone interested that the player has started playing
	...
	*/
	public function moveHead(newHeadPosition:Number):Void
	{
		...
		else
		{
			...
			if (_inMultiPathMode)
			{
				// newposition is in ms, relative to the entire playlist.				
				_playhead = _playlist.start(newPosition);
			}
			else
			{
	...
	*/
	public function getState():Object
	{
		...
		if (_inMultiPathMode)
		{
			// currently only support one song in multi path mode.
			result.trackIndex = 0;
	...
	*/
	...
	private function _beginCrossFade():Void
	{
		clearInterval(_beginCrossFadeClearID);
		...
		var nextTrack:Track = _playlist.getCurrent();
		
		_crossFadeInSound = nextTrack.getSound();
		
		_playhead.onSoundComplete = undefined;
		_crossFadeInSound.onSoundComplete = undefined;
		
		_crossFadeInSound.setVolume(0);
		_crossFadeInSound.start();
		_crossFadeVolumeStep = _volume / (CROSS_FADE_TIME / CROSS_FADE_INTERVAL_TIME);
		
		_crossFadeTimeLeft  = CROSS_FADE_TIME;
		_crossFadeClearID = setInterval(this, "_crossFadeStep", 50);
	}
	
	private function _cancelCrossFadeInProgress():Void
	{
		clearInterval(_beginCrossFadeClearID);
		clearInterval(_crossFadeClearID);
		if (_crossFadeInSound != undefined)
		{
			_crossFadeInSound.stop();
			_crossFadeInSound = undefined;
		}
	}
	
	private function _activateCrossFade():Void
	{
		_cancelCrossFadeInProgress();
		
		_beginCrossFadeClearID = setInterval(this, "_beginCrossFade", _playhead.duration - _playhead.position - (CROSS_FADE_TIME + 100));
	}
	
	private function _crossFadeStep():Void
	{
		if (_crossFadeTimeLeft < CROSS_FADE_INTERVAL_TIME)
		{
			clearInterval (_crossFadeClearID);			
			
			_playhead.setVolume(0);
			_crossFadeInSound.setVolume(_volume);
			
			_playhead.stop();
			_playhead = _crossFadeInSound;
			_crossFadeInSound = undefined;
		...
		_crossFadeTimeLeft -= CROSS_FADE_INTERVAL_TIME;
		
		var outVolume:Number = _playhead.getVolume();
		var inVolume:Number = _crossFadeInSound.getVolume();
		
		outVolume -= _crossFadeVolumeStep;
		inVolume += _crossFadeVolumeStep;
		if (outVolume < 0)
		{
			outVolume = 0;
		}
		...
		_playhead.setVolume(outVolume);
		_crossFadeInSound.setVolume(inVolume);
	}
	...
	private function _updateStats():Void
	{
		if(_state > STOPPED && _playhead.getBytesTotal() > 0)
		{
			...
			if(currentTrack.isFullyLoaded()) 
			{
				...
				if (_inMultiPathMode)
				{
					_duration = _playlist.getTotalDuration();
				}
				else
				{
			...
			if(_playhead.position > 0)
			{
				if (_inMultiPathMode)
				{
					_position = _playlist.getElapsedTime();
				}
				else
				{
	...
	*/
	...
	{
		if (_isXMLFilename(trackFileList))
		{
			_inMultiPathMode = true;
			_loadXMLPlaylist(trackFileList, titleList, artistList);
			return;
		}
		
		_inMultiPathMode = false;
		_loadTrackFileList(trackFileList, titleList, artistList);
	}
	
	private function _isXMLFilename(filename:String):Boolean
	{
		return (filename.indexOf(",") == -1 && filename.indexOf(".xml") != -1);
	}
		
	private function _loadXMLPlaylist(filename:String, titleList:String, artistList:String):Void
	{
		var parser = new MultiPathParser (filename);
		
		_isLoadingXML = true;
		
		parser.run(Delegate.create(this, function(sequence:String){								   
									   _loadTrackFileList(sequence, titleList, artistList);
									   _isLoadingXML = false;
									   if(_preloadOnFinishXML)
									   {
										   _preloadOnFinishXML = false;
										   _doPreload();
									   }
								   }));
	}
	
	private function _loadTrackFileList(trackFileList:String, titleList:String, artistList:String):Void
	{
