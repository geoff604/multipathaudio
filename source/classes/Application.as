...
class Application
{
	...
	private static var _preloadingStarted:Boolean = false;
	...
	*/
	public static function start(sourceFile:String, options:Object):Void
	{
		...
		if (ExternalInterface.available) {
			...
			//add event listeners for js
			ExternalInterface.addCallback("addListener", Application, Application.addJSListener);
			ExternalInterface.addCallback("removeListener", Application, Application.removeJSListener);
		...
		// Notification that the app has loaded//started
		if (ExternalInterface.available) {
			ExternalInterface.call("onWPPlayerReady", _options.playerID);
		}
	}
	...
	*/
	public static function onPlay(trackIndex:Number):Void
	{
		if (!_preloadingStarted)
		{
			...
			_player.preloadTracks( function () {_player.play(trackIndex, false)});
		}
		else
		{
		...
		// forward any relevent calls to js listeners
		var tempObj:Object = _player.getState();
		forward("PLAY", tempObj.trackInfo);
	...
	* called from Player.as by broadcastmessage
	*/
	public static function onMetaData():Void
	{
		var tempObj:Object = _player.getState();
		forward("METADATA", tempObj.trackInfo);
	}
	...
	private static function _update():Void
	{
		...
		switch(playerState.state)
		{
			case Player.NOTFOUND:
				...
				// changed the File Not Found message to Loading message.
				display_mc.setText(trackNumber + "Loading please wait...", 0);
			...
			default:
				...
				else
				{
					...
					if(playerState.buffering) message += "Buffering...";
						...
						if(playerState.trackInfo.artist.length > 0 || playerState.trackInfo.title.length > 0)
						{
							if(playerState.trackInfo.artist.length > 0) message += playerState.trackInfo.artist + " - ";
							message += playerState.trackInfo.title;
						}
	...
	*/
	...
	// new ei calls to add remove listeners and forward messages
	private static function addJSListener(typ:String, fcn:String):Boolean
	{
		var upperCaseType:String = typ.toUpperCase()
		listeners.push({type:upperCaseType, callee:fcn});
		return true;
	}
	
	private static function removeJSListener(typ:String, fcn:String):Boolean 
	{
		for(var i:Number=0; i<listeners.length; i++) {
			if( listeners[i]['type'] == typ && listeners[i]['callee'] == fcn) {
	...
	private static function forward(typ:String, dat:Object):Void 
	{
		if (!dat) { dat = new Object(); }
	 	dat.playerID = _options.playerID
		
		for (var itm:String in listeners) {
			if (listeners[itm]['type'] == typ) {
